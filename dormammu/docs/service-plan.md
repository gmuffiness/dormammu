...
```bash
# 시뮬레이션 목록 확인
dormammu replay list

# 특정 시뮬레이션 재생 (처음부터)
dormammu replay show --simulation-id {sim_id}

# 특정 턴부터 재생
dormammu replay show --simulation-id {sim_id} --from-turn 5

# 특정 에이전트 관점으로 재생
dormammu replay show --simulation-id {sim_id} --agent "Elena Vasquez"
```
...
dormammu replay enter --simulation-id {sim_id} --turn 7 --agent "Elena Vasquez"
...
dormammu export report --session-id {session_id} --format markdown

# 시나리오 트리를 JSON으로 내보내기
dormammu export tree --session-id {session_id}

# 특정 에이전트의 전체 일지 내보내기
dormammu export agent-diary --simulation-id {sim_id} --agent "Elena Vasquez"
```
...
```bash
# 현재 세션 상태 확인
dormammu status --session-id {session_id}

# 일시정지 (현재 시뮬레이션 완료 후)
dormammu pause --session-id {session_id}

# 재개
dormammu resume --session-id {session_id}

# 즉시 중단 (진행 중 시뮬레이션 종료)
dormammu stop --session-id {session_id} --force
```
...
```bash
# 실시간 비용 확인
dormammu cost --session-id {session_id}
...
[tool.poetry.scripts]
dormammu = "dormammu.cli.main:cli"
```
...
```bash
# MVP 실행 예시
dormammu run --topic "AGI가 등장하면 인류는 어떻게 될까?" --mode single
